      ...
      <form onSubmit={handleSubmit} className="space-y-5 bg-white rounded-2xl p-5 border border-gray-200 shadow-sm">
            ...
            {CATEGORIES.map(({ value, label, emoji }) => (
              <button key={value} type="button" onClick={() => setCategory(value)}
                className={`flex items-center gap-2 px-3 py-2.5 rounded-xl border text-sm font-medium transition text-left ${
                  category === value ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-gray-200 hover:border-gray-300 text-gray-700'
                }`}>
        ...
        <div>
          ...
          <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={3}
            ...
            className="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none resize-none text-sm" />
        </div>
          ...
          {location ? (
            <div className="flex items-start gap-3 p-3 bg-green-50 border border-green-200 rounded-xl">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-green-600 mt-0.5 truncate">{address}</p>
              </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              <button type="button" onClick={captureGPS} disabled={gpsLoading}
                className="flex items-center justify-center gap-2 px-3 py-3 border border-gray-300 rounded-xl text-gray-700 hover:border-blue-500 hover:bg-blue-50 transition text-sm font-medium">
